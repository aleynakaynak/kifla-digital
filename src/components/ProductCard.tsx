...
export function ChipCard({ product }: { product: Product }) {
  return (
    <div className="flex items-center gap-1.5 rounded-full border border-border bg-card px-4 py-2.5 text-sm text-espresso shadow-[var(--shadow-soft)] hover:border-gold/70 transition-colors">
      {product.name}
      {product.price && (
        <span className="font-sans text-sm font-medium tracking-wide text-gold">
          {product.price}
